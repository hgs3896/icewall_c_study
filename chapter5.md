...
위치 값이라는 것은 무엇일까? 파이썬과 같은 다른 프로그래밍 언어를 잘 공부했다면 우리가 흔히 변수라고 부르는 것들은 소위 이름을 통해 해당 이름에 연결되는 object\(쉽게 **어떤 값**이라 생각합시다\) 하나를 얻을 수 있는 수단으로 알고 계시겠죠? 그럼 그 object가 메모리 어디에 있는 지와 같은 **위치**는 
