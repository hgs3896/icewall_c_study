...
'int' 형식, 정수 형식이라는 의미에요.

형식은 파이썬에서도 중요하지만, C언어에서는 매우 중요해요.

일단, **함수 이름 앞에 적혀 있는 형식**은 함수가 호출된 후에 값을 호출한 함수로 돌려주는 **값을 어떻게 읽을 지**를 알려줘요.

'이게 뭔 ~~변기 속 고양이 같은~~ 말이냐?'라고 생각할 수 있어요.

그런데 생각해보면 일상에서도 말이 같다고 다 같은 의미로 사용되지는 않아요.
