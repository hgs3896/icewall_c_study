...
왜냐하면 **함수 이름 앞에 적혀 있는 형식**은 함수가 호출된 후에 값을 호출한 함수로 돌려주는 **값을 어떻게 읽을 지**를 알려줘요.
...
이렇듯 사람이 쓰는 말에도 모호함이 있는데, 컴퓨터처럼 정밀함이 요구되는 기계가 사용하는 값에는 그 의미를 명확하게 해줄 필요가 있습니다.

> 조금 더 구체적인 예를 들어보면, 숫자 **17**을 컴퓨터에서 저장할 때,
>
> 8비트\(2진수 8자리\)로 표현하면 **00010001** 으로 나타낼 수 있습니다.
>
> 그런데 이를 4비트\(2진수 4자리\)씩 끊어서 읽는다면,
>
> 0001 0001, 즉 11을 나타낼 수도 있습니다.
>
> 분명 같은 값이지만 다른 의미로 해석할 수 있는 것이죠.

\(자세한 내용은 2학년 디지털 논리 시간에 배워요\)

그래서 **형식**은 컴퓨터에게 '**값을 어떻게 해석할 지**'를 명확히 하는데 필수적인 녀석입니다.

우리는 형식을 잘 다룰수록 더 정확하게 내가 원하는 바를 컴퓨터에게 전달할 수 있을거에요.
