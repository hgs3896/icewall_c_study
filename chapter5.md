...
그렇다면, printf\("%s", str\);의 결과는? ~~어떻게 될지 아무도 장담할 수 없어요.~~
...
```
char str1[6] = { 'k', 'o' ,'r' ,'e', 'a', '\n' };
char str2[6] = { 'k', 'o' ,'r' ,'e', 'a', '\0' }; // char str2[6] = "korea"; 와 동일한 선언 및 초기화에요.
```
...
str1의 경우 문자 배열의 끝을 알 수 없지만, str2의 경우 널문자\(\0\)을 통해 5자임을 알 수 있다.

**그렇기 때문에 문자열을 담는 char 배열의 크기는 항상 \(문자개수+1\) 이상이어야 한다.**

파이썬에서 str형식과 다르게, ....

C에서 문자열의 처리는 선언과 초기화를 어떻게 하느냐에 따라 immutable할 지 mutable할 지가 결정돼요.

```
// Mutable한 경우
char str[] = "hello"; // char str[6] = "hello";와 동일
char str[6] = "hello";

// Immutable한 경우
const char str[] = "hello";
char * str = "hello";
char const * str = "hello"; // const char *str = "hello";와 동일
char * const str = "hello";
char const * const str = "hello"; // const char * const str = "hello";와 동일
```
...
-&gt;문자열 두개를 비교해 같으면 0을, 문자열1이 알파벳 및 숫자 순서\(lexicographical order\)상 작으면 음수, 문자열2가 크면 양수를 반환합니다.
...
##### 지금까지 배열 및 문자열에 대해 알아보았습니다. {#지금까지-1차원-2차원-배열-char-배열에-대해-알아보았다}
